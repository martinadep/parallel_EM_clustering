...
do
    ...
    OMP_NUM_THREADS=${THREADS} "${EXECUTABLE_PATH}" -d "${DATASET_PATH}" -k "${K}" -o "${RESULTS_DIR}/em_P${N}_K${K}_D${D}_threads${THREADS}.csv" >> "${RESULTS_DIR}/strong_scaling_omp_results_P${N}_K${K}_D${D}.txt"
done
