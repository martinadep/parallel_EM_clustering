#!/bin/bash
# Script per testare scaling K=15 in sessione interattiva

# 1. Carica i moduli necessari
module load gcc91 openmpi-4.0.4

# 2. Imposta i percorsi relativi alla directory corrente (root del progetto)
PROJECT_ROOT=$(pwd)
...
run_test() {
    ...
    TMP_LOG="interactive_run_${CORES}_K15.log"
    
    # Esegue MPI con flag di sicurezza per la connessione
